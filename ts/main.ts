 ...
 */
const deal = (deck: Deck, player1: Player, player2: Player, amount: number) => {
 ...
 */
const reportScore = (player1: Player, player2: Player) => {
 ...
 */
const checkWin = (player1: Player, player2: Player) => {
    let winner = null;
    ...
    if (winner) {
        ...
        console.log(`Final score: ${reportScore(player1, player2)}`);
        process.exit(0);
    }
};
 ...
 */
const countPegPoints = (playedCards: Card[], verbose = false) => {
    let points = 0;
    ...
    for (const i of [
        ...
        if (playedCards[i].rank === playedCards[playedCards.length - 1].rank) {
            pairs += 1;
        } else {
            break;
        }
...
const addPoints = (
    ...
    if (report) {
        console.log(`The score is now ${reportScore(player, otherPlayer)}`);
    }
};
...
const main = async () => {
    ...
    while (true) {
        ...
        console.log(`${dealer.getName()} threw ${dealerThrew} into the crib.`);
        ...
        if (cutCard.rank == 11) {
            ...
            addPoints(dealer, 2, pone);
            checkWin(dealer, pone);
        }

        // keep track of count points
        const dealerPoints = new Hand(dealer.getHandCards(), cutCard).count();
        ...
        while (true) {
            ...
            if (playedCard) {
                console.log(`${playerToPlay.getName()} plays ${playedCard}.`);
                ...
                const pegPoints = countPegPoints(playedCards);
                if (pegPoints != 0) {
                    console.log(
                ...
                if (
                    ...
                    addPoints(playerToPlay, 1, otherPlayer(playerToPlay));
                    checkWin(dealer, pone);
                    break;
                }
                if (!passed) {
        ...
        addPoints(pone, ponePoints, dealer, false);
        checkWin(dealer, pone);

        console.log(`${dealer.getName()} counts ${dealerPoints} points.`);
        addPoints(dealer, dealerPoints, pone, false);
        checkWin(dealer, pone);

        console.log(
            `${dealer.getName()} scores ${cribPoints} points from the crib.`,
        );
        addPoints(dealer, cribPoints, pone);
        checkWin(dealer, pone);
